...
public sealed class ClientApplicationLocaleAuthorization(AuthorizationScope _scope)
	: IClientApplicationLocaleAuthorization
{
	public async ValueTask<bool> CanCreateAsync(ClientApplicationLocaleDto dto, ClaimsPrincipal user, CancellationToken cancellationToken = default)
		=> await CanManageApplicationAsync(dto.ClientApplicationGuid, cancellationToken);

	public async ValueTask<bool> CanReadAsync(Guid clientApplicationGuid, string localeKey, ClaimsPrincipal user, CancellationToken cancellationToken = default)
		=> await CanManageApplicationAsync(clientApplicationGuid, cancellationToken);

	public async ValueTask<bool> CanCountAsync(ClientApplicationLocaleFilter filter, ClaimsPrincipal user, CancellationToken cancellationToken = default)
		=> await RestrictListAsync(filter, cancellationToken);

	public async ValueTask<bool> CanListAsync(ClientApplicationLocaleFilter filter, ClaimsPrincipal user, CancellationToken cancellationToken = default)
		=> await RestrictListAsync(filter, cancellationToken);

	public async ValueTask<bool> CanUpdateAsync(Guid clientApplicationGuid, string localeKey, ClientApplicationLocaleDto dto, ClaimsPrincipal user,
	...
	public async ValueTask<bool> CanDeleteAsync(Guid clientApplicationGuid, string localeKey, ClaimsPrincipal user, CancellationToken cancellationToken = default)
		=> await CanManageApplicationAsync(clientApplicationGuid, cancellationToken);

	public async ValueTask<bool> CanReadByClientApplicationGuidAsync(Guid key, ClaimsPrincipal user, CancellationToken cancellationToken = default)
